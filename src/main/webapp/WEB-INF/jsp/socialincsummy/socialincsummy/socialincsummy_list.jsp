...
<head>
	...
	<body class="no-skin">
					...
					<div class="page-content">
						...
						<div class="page-header">
										...
										<span class="label label-xlg label-yellow arrowed-in arrowed-right"
										    id="subTitle" style="margin-left: 2px;">社保汇总</span> 
	    ...
	    function getSelectBillCodeOptions(){
			...
			$.ajax({
			    type: "POST",
				url: '<%=basePath%>socialincsummy/getBillCodeList.do?'
			...
			$.ajax({
				...
				success: function(response){
					if(response.code==0){
			            ...
			            var childGridURL = '<%=basePath%>socialincsummy/getFirstDetailList.do?'
				            +'SelectedCustCol7='+$("#SelectedCustCol7").val()
				            +'&SelectedDepartCode='+$("#SelectedDepartCode").val()
			...
			$.ajax({
				...
				success: function(response){
					if(response.code==0){
			            ...
			            var childGridPagerID = parentRowID + _pager;
			            // send the parent row primary key to the server so that we know which grid to show
			            var childGridURL = '<%=basePath%>socialincsummy/getSecondDetailList.do?';
			            //childGridURL = childGridURL + "&parentRowID=" + encodeURIComponent(parentRowKey)
			...
			if(!(ids!=null && ids.length>0)){
	            ...
	            bootbox.confirm(msg, function(result) {
					if(result) {
						...
						$.ajax({
							type: "POST",
							url: '<%=basePath%>socialincsummy/deleteAll.do?',
