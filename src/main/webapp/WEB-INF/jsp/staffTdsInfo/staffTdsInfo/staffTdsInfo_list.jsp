...
<body class="no-skin">
				...
				<div class="page-content">
					...
					<div class="page-header">
									...
									<span class="label label-xlg label-yellow arrowed-in arrowed-right"
									    id="subTitle" style="margin-left: 2px;">个税扣除项导入</span> 
	...
	<script type="text/javascript"> 
    		...
    		if(!(ids!=null && ids.length>0)){//
                ...
                bootbox.confirm(msg, function(result) {
    				if(result) {
    					...
    					$.ajax({
    						type: "POST",
    						url: '<%=basePath%>staffTdsInfo/deleteAll.do?'
        	...
        	if(!(ids!=null&&ids.length>0)){
                ...
                bootbox.confirm(msg, function(result) {
        			if(result) {
        				...
        				$.ajax({
        					type: "POST",
        					url: '<%=basePath%>staffTdsInfo/updateAll.do?'
        						+ 'SelectedBusiDate='+$("#SelectedBusiDate").val()
        	                    +'&ShowDataBusiDate='+ShowDataBusiDate,
        			    	data: {DataRows:JSON.stringify(listData)},
        					dataType:'json',
        ...
        function importItems(){
    	   ...
    	   diag.URL = '<%=basePath%>staffTdsInfo/goUploadExcel.do?'
    			+ 'SelectedBusiDate='+$("#SelectedBusiDate").val()
                + '&ShowDataBusiDate='+ShowDataBusiDate;
    	   diag.Width = 300;
    	   diag.Height = 150;
        ...
        function exportItems(){
        	window.location.href='<%=basePath%>staffTdsInfo/excel.do?'
        		+ 'SelectedBusiDate='+$("#SelectedBusiDate").val()
                +'&ShowDataBusiDate='+ShowDataBusiDate;
        }
	    ...
	    function tosearch() {
			...
			$(gridBase_selector).jqGrid('setGridParam',{  // 重新加载数据
    			url: '<%=basePath%>staffTdsInfo/getPageList.do?'
            		+ 'SelectedBusiDate='+$("#SelectedBusiDate").val()
                    + '&ShowDataBusiDate='+ShowDataBusiDate,
    			editurl: '<%=basePath%>staffTdsInfo/edit.do?'
            		+ 'SelectedBusiDate='+$("#SelectedBusiDate").val()
                    + '&ShowDataBusiDate='+ShowDataBusiDate
							}).trigger("reloadGrid");
		}  
